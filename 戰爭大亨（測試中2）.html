...
<body class="text-gray-200 font-sans min-h-screen">
    ...
    <!-- 登录/注册模态框 -->
    <div id="auth-modal" class="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
        <div class="auth-modal w-full max-w-md rounded-2xl shadow-2xl overflow-hidden">
            <div class="p-6">
                <div class="flex justify-center gap-6 mb-6 border-b border-gray-700 pb-2">
                    <div id="login-tab" class="auth-tab active text-white font-semibold pb-2">登录</div>
                    <div id="register-tab" class="auth-tab text-gray-400 pb-2">注册</div>
                </div>
                <!-- 登录表单 -->
                <div id="login-form" class="space-y-4">
                    <div><label class="block text-sm mb-1 text-gray-300">电子邮箱</label><input type="email" id="login-email" class="auth-input w-full" placeholder="your@email.com"></div>
                    <div><label class="block text-sm mb-1 text-gray-300">密码</label><input type="password" id="login-password" class="auth-input w-full" placeholder="输入密码"></div>
                    <div class="flex justify-between items-center">
                        <label class="checkbox-label"><input type="checkbox" id="remember-me"> <span>记住我</span></label>
                    </div>
                    <button id="login-btn" class="auth-btn w-full text-white">登录</button>
                </div>
                <!-- 注册表单 -->
                <div id="register-form" class="space-y-4 hidden">
                    <div><label class="block text-sm mb-1 text-gray-300">昵称 / 名字</label><input type="text" id="reg-name" class="auth-input w-full" placeholder="如何称呼您？"></div>
                    <div><label class="block text-sm mb-1 text-gray-300">电子邮箱</label><input type="email" id="reg-email" class="auth-input w-full" placeholder="your@email.com"></div>
                    <div><label class="block text-sm mb-1 text-gray-300">密码</label><input type="password" id="reg-password" class="auth-input w-full" placeholder="设置密码"></div>
                    <div><label class="block text-sm mb-1 text-gray-300">确认密码</label><input type="password" id="reg-confirm" class="auth-input w-full" placeholder="再次输入密码"></div>
                    <button id="register-btn" class="auth-btn w-full text-white">注册</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div id="main-app" style="display: none;">
        <nav class="sticky top-0 z-40 glass-nav px-4 py-3">
            <div class="max-w-6xl mx-auto flex items-center justify-between">
                ...
                <div class="flex items-center gap-3">
                    <button onclick="openTrackerModal()" class="relative px-3 py-1.5 text-xs font-semibold bg-blue-600/30 text-blue-200 border border-blue-500/40 rounded-lg hover:bg-blue-600/50 transition backdrop-blur-sm">
                    ...
                    <button onclick="openAchievementModal()" class="px-3 py-1.5 text-xs font-semibold bg-amber-600/30 text-amber-200 border border-amber-500/40 rounded-lg hover:bg-amber-600/50 transition backdrop-blur-sm">
                        <i class="fa-solid fa-trophy mr-1"></i> 成就
                    </button>
                    <button id="logout-btn" class="px-3 py-1.5 text-xs font-semibold bg-red-600/30 text-red-200 border border-red-500/40 rounded-lg hover:bg-red-600/50 transition backdrop-blur-sm">
        ...
        <header class="relative pt-10 pb-6 px-4 text-center">
            <div class="max-w-3xl mx-auto space-y-3">
                ...
                <p class="text-xs md:text-sm text-gray-300 max-w-lg mx-auto">现代化任务网站 | 坦克 | 飞机 | 直升机 | 船舰</p>
            </div>
            <div class="max-w-md mx-auto mt-6 px-2">
                <div class="flex items-center gap-2">
                    <input type="text" id="search-input" oninput="searchTasks()" placeholder="输入「任务/载具名称」进行精确搜索..." class="w-full bg-black/40 backdrop-blur-md border border-gray-600 rounded-xl py-3 px-4 text-sm text-white placeholder-gray-400 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition">
                    <button onclick="showEasterEggPage(); unlockAchievement('easter_egg')" class="bg-gradient-to-r from-amber-500/30 to-orange-500/30 hover:from-amber-500/50 hover:to-orange-500/50 border border-amber-500/50 rounded-xl py-3 px-4 backdrop-blur-sm transition"><i class="fa-solid fa-dragon text-amber-400 text-xl"></i></button>
                </div>
        ...
        <main class="max-w-6xl mx-auto px-4 mt-2 space-y-10">
            ...
            <section id="category-selector" class="space-y-4">
                <h2 class="text-base font-bold text-gray-300">选择任务分类</h2>
                <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
                    <div onclick="scrollToSection('tank-section'); unlockAchievement('tank_category')" class="rounded-2xl bg-gradient-to-br from-blue-600/60 to-indigo-700/60 p-4 cursor-pointer transform hover:-translate-y-1 transition duration-300 shadow-lg backdrop-blur-sm border border-white/20"><div class="text-white"><h3 class="text-lg font-black">坦克任务</h3><p class="text-xs opacity-90">16 个项目</p></div></div>
                    <div onclick="scrollToSection('helicopter-section'); unlockAchievement('helicopter_category')" class="rounded-2xl bg-gradient-to-br from-purple-600/60 to-violet-700/60 p-4 cursor-pointer hover:-translate-y-1 transition backdrop-blur-sm border border-white/20"><div class="text-white"><h3 class="text-lg font-black">直升机任务</h3><p class="text-xs opacity-90">10 个项目</p></div></div>
                    <div onclick="scrollToSection('airplane-section'); unlockAchievement('airplane_category')" class="rounded-2xl bg-gradient-to-br from-pink-600/60 to-rose-700/60 p-4 cursor-pointer hover:-translate-y-1 transition backdrop-blur-sm border border-white/20"><div class="text-white"><h3 class="text-lg font-black">飞机任务</h3><p class="text-xs opacity-90">17 个项目</p></div></div>
                    <div onclick="scrollToSection('hovercraft-section'); unlockAchievement('hovercraft_category')" class="rounded-2xl bg-gradient-to-br from-cyan-600/60 to-teal-700/60 p-4 cursor-pointer hover:-translate-y-1 transition backdrop-blur-sm border border-white/20"><div class="text-white"><h3 class="text-lg font-black">悬空载具</h3><p class="text-xs opacity-90">6 个项目</p></div></div>
                    <div onclick="scrollToSection('ship-section'); unlockAchievement('ship_category')" class="rounded-2xl bg-gradient-to-br from-emerald-600/60 to-teal-800/60 p-4 cursor-pointer hover:-translate-y-1 transition backdrop-blur-sm border border-white/20"><div class="text-white"><h3 class="text-lg font-black">船舰与潜艇</h3><p class="text-xs opacity-90">7 个项目</p></div></div>
                </div>
                <div class="mt-4"><a href="https://wttd.trade/" target="_blank" onclick="unlockAchievement('trade_market')" class="block rounded-2xl bg-gradient-to-br from-slate-700/70 to-slate-900/70 p-4 text-center max-w-xs backdrop-blur-sm border border-white/20 hover:border-blue-500 transition"><h3 class="text-lg font-black text-white">⚔ 交易市场 (wttd.trade)</h3></a></div>
            </section>
            ...
            <section id="social-section" class="max-w-xl space-y-4">
                ...
                <div class="space-y-3">
                    <a href="https://roblox.com/join/y54bw" target="_blank" onclick="unlockAchievement('social_community')" class="social-link-card"><i class="fa-solid fa-gamepad mr-4 text-blue-500 text-xl"></i> War Tycoon (Roblox)</a>
                    <a href="https://discord.gg/nFeyYDySug" target="_blank" onclick="unlockAchievement('social_community')" class="social-link-card"><i class="fa-brands fa-discord mr-4 text-indigo-400 text-xl"></i> Discord</a>
                    <a href="https://tiktok.com/@wartycoon" target="_blank" onclick="unlockAchievement('social_community')" class="social-link-card"><i class="fa-brands fa-tiktok mr-4 text-pink-500 text-xl"></i> Tik Tok</a>
                    <a href="https://x.com/WarTycoonRBLX" target="_blank" onclick="unlockAchievement('social_community')" class="social-link-card"><i class="fa-brands fa-x-twitter mr-4 text-white text-xl"></i> X (Twitter)</a>
                    <a href="https://www.youtube.com/@WarTycoonRBLX" target="_blank" onclick="unlockAchievement('social_community')" class="social-link-card"><i class="fa-brands fa-youtube mr-4 text-red-500 text-xl"></i> YouTube</a>
                </div>
            </section>

            <!-- 派系历史完整版 -->
            <hr id="faction-hr" class="border-gray-700 my-6" />
            <section id="faction-section" class="glass-card p-5 md:p-6 space-y-5">
                <div class="flex flex-wrap items-center justify-between gap-2 border-b border-blue-500/30 pb-3"><h2 class="text-xl md:text-2xl font-black text-white flex items-center gap-2"><i class="fa-solid fa-landmark text-blue-400"></i> 派系 · 大兴王朝历史<span class="text-xs bg-blue-500/30 text-blue-300 px-2 py-0.5 rounded-full">2023-至今</span></h2></div>
                <div class="flex flex-wrap gap-3 text-sm"><span class="era-badge px-3 py-1 rounded-full text-blue-300 text-xs font-mono">🏛️ 【古代史】2023.01.29-2024.7.14</span><span class="era-badge px-3 py-1 rounded-full text-purple-300 text-xs font-mono">📜 【近代史】2025.06.27-2026.1.20</span><span class="era-badge px-3 py-1 rounded-full text-pink-300 text-xs font-mono">⚡ 【现代史】2026.02.11-至今</span><span class="bg-gradient-to-r from-amber-500/30 to-rose-500/30 border border-amber-500/50 px-3 py-1 rounded-full text-amber-300 text-xs font-bold">✿ BNS鼎盛时期 2025.08.21-2026.01.05 ✿</span></div>
            ...
            <!-- 关于我们完整版 -->
            <hr id="about-hr" class="border-gray-700 my-6" />
            <section id="aboutus-section" class="glass-card p-5 md:p-6">
                ...
                <div class="space-y-4 text-gray-300">
                    <p class="bg-black/30 rounded-xl p-4 border-l-4 border-blue-500 text-sm leading-relaxed backdrop-blur-sm">✿ <span class="font-bold text-blue-300">2026.05.26</span> 管理员濠制作了派系网页2.0 (重制版)，墨墨决定重新更新派系网页。<br>因1.0的经验，2.0的功能将更加全面，同时档案大小也是指数增加，我们将推翻旧的UI设计，制定一个全新的UI以及动画设计等，理想情况会加入注册登录功能。<br>后续我们团队会想办法将其弄成软件或网站，敬请期待 ✨<br>如果你也想为这个做贡献，就加入我们 Harma工作室™️ 吧！</p>
                    <div class="flex flex-wrap gap-3 text-sm font-mono"><span class="bg-pink-500/20 text-pink-300 px-3 py-1 rounded-full text-xs">💖 “If you love Harma, I love you!”</span><span class="bg-amber-500/20 text-amber-300 px-3 py-1 rounded-full text-xs">🍑 “If you love Momo, I love you!”</span></div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3 bg-black/30 p-3 rounded-xl backdrop-blur-sm"><div class="flex items-center gap-2 text-sm"><i class="fa-regular fa-envelope text-blue-400"></i> <span class="break-all">ag6428570@gmail.com</span> <span class="text-blue-300 text-xs">—— GeoffreyLei (技术人员)</span></div><div class="flex items-center gap-2 text-sm"><i class="fa-regular fa-envelope text-pink-400"></i> <span class="break-all">1710278186@qq.com</span> <span class="text-pink-300 text-xs">—— 𝑴𝒐𝒎𝒐 (CEO)</span></div></div></div>
                    <p class="text-xs text-gray-500 text-right italic">「千年王朝之梦，战火淬炼新生」</p>
                </div>
            </section>

            <!-- 任务列表渲染区 -->
            <hr id="section-divider" class="border-gray-700 my-6" />
            <div id="tasks-container" class="space-y-12"></div>
        </main>
    </div>

    <!-- 成就模态框 -->
    <div id="achievement-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md opacity-0 pointer-events-none transition-all duration-200">
        <div class="bg-darkCard/80 border border-gray-700 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden transform transition-all duration-200 achievement-modal-content backdrop-blur-xl">
            <div class="p-5">
    ...
    <!-- 彩蛋页面 -->
    <div id="easteregg-page" class="fixed inset-0 z-50 easter-egg-bg hidden overflow-y-auto">
        <div class="min-h-screen flex flex-col items-center justify-center p-6 relative">
            <div class="max-w-5xl w-full space-y-10">
                <div class="glass-card p-6 shadow-2xl">
                    <div class="bg-gradient-to-r from-slate-800/80 to-gray-900/80 px-4 py-2 border-b border-indigo-500/30 flex items-center justify-between rounded-t-2xl backdrop-blur-sm">
                ...
                <div class="glass-card p-6 flex flex-col items-center border border-pink-500/30">
                    ...
                    <div class="text-center space-y-2"><p class="font-mono text-pink-300 text-sm">👑 首席执行官 👑</p><p class="text-2xl font-bold text-white">CloverMo</p><p class="text-amber-300 text-sm">Harma工作室™️ CEO</p><div class="mt-3 px-4 py-2 bg-amber-500/20 rounded-full inline-block border border-amber-500/30 backdrop-blur-sm"><p class="text-amber-300 text-xs italic">「执码领团，破界拓新」</p></div></div>
                </div>
    ...
    <!-- 任务详情与追踪模态框 -->
    <div id="detail-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-200"><div class="bg-darkCard/90 border border-gray-700 w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden transform scale-95 transition-all duration-200 backdrop-blur-xl"><div id="modal-header-accent" class="h-2 w-full bg-blue-500"></div><div class="p-6 space-y-4"><div class="flex justify-between items-start"><div><span id="modal-category" class="text-xs font-semibold px-2 py-0.5 rounded bg-gray-800/70 text-gray-400">分类</span><h3 id="modal-title" class="text-xl font-bold text-white mt-1">名称</h3></div><button onclick="closeModal()" class="text-gray-400 hover:text-white text-xl">×</button></div><div class="bg-gray-900/70 border border-gray-800 rounded-xl p-4"><p class="text-xs text-gray-500 uppercase tracking-wider mb-1 font-bold">解锁获取条件</p><p id="modal-requirement" class="text-sm text-gray-100 font-semibold leading-relaxed">要求详情</p></div><div class="flex space-x-2 pt-2"><button id="modal-track-btn" onclick="toggleTrackCurrent()" class="flex-1 bg-blue-600/80 hover:bg-blue-500 text-white font-bold py-2 px-4 rounded-xl text-sm transition">加入追踪</button><button onclick="copyCurrentRequirement()" class="bg-gray-800/80 hover:bg-gray-700 text-gray-200 font-bold py-2 px-4 rounded-xl text-sm transition">复制</button></div><p id="modal-toast" class="text-xs text-center text-green-400 font-medium opacity-0 transition-opacity duration-200"></p></div></div></div>
    <div id="tracker-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-200"><div class="bg-darkCard/90 border border-gray-700 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden transform scale-95 transition-all duration-200 backdrop-blur-xl"><div class="p-6 space-y-4"><div class="flex justify-between items-center"><h3 class="text-lg font-bold text-white">我的资料追踪清单</h3><button onclick="closeTrackerModal()" class="text-gray-400 hover:text-white text-xl">×</button></div><div id="tracker-list-content" class="space-y-2 max-h-60 overflow-y-auto pr-1"></div><div class="border-t border-gray-800 pt-4 flex justify-between items-center"><span class="text-xs text-gray-500">提示：资料已储存于浏览器缓存中。</span><button onclick="clearAllTracked()" class="text-xs text-red-400 hover:text-red-300">全部清除</button></div></div></div></div>
    ...
    <script>
        ...
        function saveUsers() {
            localStorage.setItem('war_tycoon_users', JSON.stringify(users));
        }
        ...
        function simulateSendEmail(email, name, type) {
            const msg = type === 'welcome' ? `🎉 欢迎注册，${name}！\n演示模式：已向 ${email} 发送欢迎邮件。\n请使用您设置的密码登录。` : `🔐 演示模式：登录通知已发送至 ${email}。`;
            alert(msg);
        }
        ...
        document.getElementById('login-btn').addEventListener('click', () => {
            ...
            localStorage.setItem('war_tycoon_user', email);
            localStorage.setItem('war_tycoon_name', user.name);
            if (remember) {
        ...
        document.getElementById('register-btn').addEventListener('click', () => {
            ...
            // 模拟发送欢迎邮件
            simulateSendEmail(email, name, 'welcome');
            ...
            // 自动切换到登录标签，并预填邮箱
            loginTab.click();  // 触发切换到登录选项卡
            document.getElementById('login-email').value = email;
            document.getElementById('login-password').value = '';
            alert('注册成功！请使用您的密码登录。');
        });
        ...
        function renderTrackerList() { const content = document.getElementById('tracker-list-content'); if (trackedTasks.length === 0) { content.innerHTML = `<p class="text-center py-6 text-xs text-gray-500">暂无追踪项目</p>`; return; } content.innerHTML = trackedTasks.map(t => `<div class="p-2.5 bg-gray-900/70 border border-gray-800 rounded-lg flex items-center justify-between text-xs"><div><span class="text-[10px] text-blue-400 font-bold">[${t.category}]</span><span class="text-white font-bold">${t.name}</span><p class="text-gray-400 mt-1">${t.requirement}</p></div><button onclick="toggleTrackDirectly('${t.name}'); renderTrackerList();" class="text-red-400 px-2 font-bold">×</button></div>`).join(''); }
